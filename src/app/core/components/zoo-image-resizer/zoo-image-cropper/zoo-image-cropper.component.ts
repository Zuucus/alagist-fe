...
@Component({
  selector: 'zoo-image-cropper',
  templateUrl: './zoo-image-cropper.component.html',
  styleUrls: ['./zoo-image-cropper.component.scss'],
})
export class ZooImageCropperComponent implements OnChanges {
  ...
  private resizeCropperPosition() {
    const displayedImage =
      this.elementRef.nativeElement.querySelector('.source-image');
    if (
  ...
  private cropToOutputType(
    ...
    switch (this.outputType) {
      case 'base64':
        ...
        break;
      case 'file':
        this.cropToFile(cropCanvas).then((result: Blob | null) => {
          output.file = result;
          this.imageCropped.emit(output);
        });
        break;
